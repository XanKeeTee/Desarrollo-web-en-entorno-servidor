    ...
    <div class="container mt-4">
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">Generador de Sugerencias de Menú</h5>
            </div>
            <div class="card-body">
                <form method="POST">
                    <div class="mb-3">
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($numMenus > 0) {
                ...
                for ($i = 1; $i <= $numMenus; $i++) {
                    $entranteSugerido = $menu['entrante'][array_rand($menu['entrante'])];
                    $primeroSugerido = $menu['primero'][array_rand($menu['primero'])];
                    $segundoSugerido = $menu['segundo'][array_rand($menu['segundo'])];
                    $postreSugerido = $menu['postre'][array_rand($menu['postre'])];
                ...
                echo '<div class="alert alert-warning">Por favor, introduce un número válido de menús (mayor que cero).</div>';
            }
        }
        ?>
    </div>
