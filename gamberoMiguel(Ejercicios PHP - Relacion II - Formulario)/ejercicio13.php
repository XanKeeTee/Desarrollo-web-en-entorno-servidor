...
<body>
    <div class="container mt-4">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0 ">Ejercicio 13</h5>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <form method ="POST" id="ejercicio13Formulario">
                    <div class="mb-3">
    ...
    <script>
        const formElement = document.getElementById('ejercicio13Formulario');

        if (formElement) {
            formElement.addEventListener('submit', function(e) {
                const formData = new FormData(formElement);

                const inicial = Number(formData.get('inicial'));
                const primera = Number(formData.get('primera'));
                const segunda = Number(formData.get('segunda'));
                const tercera = Number(formData.get('tercera'));

                const resultadoEl = document.getElementById('resultado13');

                if (isNaN(inicial) || isNaN(primera) || isNaN(segunda) || isNaN(tercera)) {
                    e.preventDefault();
                    resultadoEl.innerHTML = '<div class="text-danger">Introduce valores numéricos válidos.</div>';
                    return;
                }

                if (inicial > 10 || primera > 10 || segunda > 10 || tercera > 10) {
                    e.preventDefault();
                    resultadoEl.innerHTML = '<div class="text-danger">Los números no pueden ser mayores a 10.</div>';
                    return;
                }
